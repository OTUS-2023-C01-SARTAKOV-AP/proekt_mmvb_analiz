...
Это связано с QUIK терминалом и его невозможностью запустить в полноценном режиме в Linux (даже испольую программу Wine - нет поддержки ODBC протокола). При этом БД PostgreSQL гораздо быстрее работает именно на Debian операционной системе (отличия явные). \
Виртуальные машины так же не подошли. При работе с тиковыми данными в QUIK торговом терминале - они просто потдармаживают, особенно во время сильных движений на рынке. Виртуальная машина оказалась критически медленной в моих целях (пробовал 2 самые ходовые). Старый ноутбук 2013 года с родной windows на борту оказался в несколько раз быстрее (ssd диск) чем виртуальная машина с Windows на новом быстром наутбуке на Debian 11 (картинки в папке "img" текущего пректа на github.com). 

Поэтому и получиась такая странная распеделённая конструкция по сети.
<p> &nbsp; </p> 
<p> &nbsp; </p> 
...
Я даже позволил себе сделать задержку в конце каждого цикла, чтобы главный цикл длился 0.5 секуды, т.к быстрее не нужно. Такая скорость принятия решений более чем устраивает. Кроме этого железо работает практически в режиме простоя, что благотворно сказывается на энергопотреблении, особенно в режиме отключения электроэнергии (и такое бывает).
...
Это был единственный случай, когда я попытался создать несколько паралельных потоков внутри БД при работе внутри одного логического кода (программы).
...
К этому проекту я шёл почти 9 лет (с конца 2014 года). Считаю, что данный проект на Си полностью реализовал возложенные на него надежды. Более того, он в разы презвошел все самые смелые ожидания.
<p> &nbsp; </p> 
<p> &nbsp; </p> 
