...
Встала задача делать огромные вычисления в несколько потоков. \
...
Сам по себе он довольно-таки медленный, а если хочешь быстрее - нужно изучать кучу вспомогательных к нему приложений. 
...
Но в 2021 году очень сильно было "надо" при работе с большими объемами вычислений. Помня всё свои ранние опыты - решил действовать кардинально: **изучить Си, как очень быстрый по скорости работы и минимальным ресурсам в работе**. 
...
> * Переводить минутки в заданные тайм фреймы: м1, м2, м3, м4, м5, м6, м10,м12, м15, м20, м30. Опять же в многопоточном режиме.
...
Одно поточный режим, выборка из БД исторических данных и укрупнение тайм фреймов по 165 акциям для генерации 11 тайм фреймов: 
![Однопоточный режим, выборка из БД по 165 акциям для генерации 11 тайм фреймов](https://github.com/OTUS-2023-C01-SARTAKOV-AP/proekt_mmvb_analiz/blob/main/img/th_1__gen_tf.png)

Здесь картина радует глаз. В одно поточном режиме программа на Си делает выборки из БД Postgres и компоновки тайм фреймов за 7 секунд ВСЕГО. Похожие расчеты в БД делались около 12-14 минут.
Разница составляет 100 раз! 

<p> &nbsp; </p> 

Выборка в 6 потоков из БД исторических данных и укрупнение тайм фреймов по 165 акциям для генерации 11 тайм фреймов: 
![Выборка в 6 потоков из БД по 165 акциям для генерации 11 тайм фреймов](https://github.com/OTUS-2023-C01-SARTAKOV-AP/proekt_mmvb_analiz/blob/main/img/th_6__gen_tf.png)

<p> &nbsp; </p> 

Выборка в 12 потоков из БД исторических данных и укрупнение тайм фреймов по 165 акциям для генерации 11 тайм фреймов: 
![Выборка в 12 потоков из БД по 165 акциям для генерации 11 тайм фреймов](https://github.com/OTUS-2023-C01-SARTAKOV-AP/proekt_mmvb_analiz/blob/main/img/th_12__gen_tf.png)
 ...
 **Выборка из БД и расчет индикаторов** (полный аналог ночному коду в БД, который работает 15 минут). Одно поточный режим в Си программе. Выборка по 165 акциям для генерации всех тайм фреймов: 
![Одно поточный режим, выборка и расчеты.](https://github.com/OTUS-2023-C01-SARTAKOV-AP/proekt_mmvb_analiz/blob/main/img/extren_start_01.png)

То есть в одно поточном режиме программы Си, ВСЕ расчёты занимают примерно 17 секунд. В БД те же самые расчеты занимают от 14 до 15 минут. Итого, чистая разница в
производительности составляет для ОДНО ПОТОЧНОГО режима более 50 раз ! (17 секунд и от 14 минут).

<p> &nbsp; </p> 


**Выборка из БД и расчет индикаторов** (полный аналог ночному коду в БД, который работает 15 минут). Запуск 6 потоков в Си программе. Выборка по 165 акциям для генерации всех тайм фреймов: 
...
**Выборка из БД и расчет индикаторов** (полный аналог ночному коду в БД, который работает 15 минут). Запуск 12 потоков в Си программе. Выборка по 165 акциям для генерации всех тайм фреймов: 
...
**Выборка из БД и расчет индикаторов**, код, который работает 15 минут. Выборка по 165 акциям для генерации всех тайм фреймов: 
...
**Кроме этого, оказалось, что Си не только быстрый, но и очень экономный в ресурсах**. На весь проект со всеми наваротами (полная, не публичная версия проекта), уходит всего 120 Мб памяти. PostgreSQL на те же вычисления полностью съедает всю отведённую ему память, а это около 5 Гб. Это хорошо видно по статистике нагрузки ПК из второго проекта (ссылка на второй проект чуть ниже). \
...
В проекте используется два ПК: один Windows, второй Debian 11. \
Это связано с QUIK терминалом и его невозможностью запустить в полноценном режиме в Linux (даже используя программу Wine - нет поддержки ODBC протокола). При этом БД PostgreSQL гораздо быстрее работает именно на Debian операционной системе (отличия явные). \
Виртуальные машины так же не подошли. При работе с тиковыми данными в QUIK торговом терминале - они просто потдормаживают, особенно во время сильных движений на рынке. Виртуальная машина оказалась критически медленной в моих целях (пробовал 2 самые ходовые). Старый ноутбук 2013 года с родной windows на борту оказался в несколько раз быстрее (ssd диск) чем виртуальная машина с Windows на новом быстром ноутбуке на Debian 11 (картинки в папке "img" текущего проекта на github.com). 

Поэтому и получилась такая странная распределённая конструкция по сети.
<p> &nbsp; </p> 
<p> &nbsp; </p> 
...
Проект реализовывался с начала сентября 2023 г по 7-9 часов в день. Это был второй мой крупный проект. Открытая часть проекта была закончена в середине ноября 2023. Дальше шли работы над закрытой частью проекта. Найденные ошибки и некоторые улучшения кода переносились из закрытой части в общедоступную версию проекта.

Проект на Си считаю средней сложности (3.5 из 5 звезд сложности). При этом с БД PostgreSQL уже хорошо и плотно работал почти 7 лет. С HTML версткой работал уже лет 20. С Питоном года 4. \
...
К тому же - основной цикл программы длится 0.20-0.36 сек, что в разы меньше желаемых 5 секунд на полный цикл. \
Я даже позволил себе сделать задержку в конце каждого цикла, чтобы главный цикл длился 0.5 секунды, т.к быстрее не нужно. Такая скорость принятия решений более чем устраивает. Кроме этого железо работает практически в режиме простоя, что благотворно сказывается на энергопотреблении, особенно в режиме отключения электроэнергии (и такое бывает).
...
Это был единственный случай, когда я попытался создать несколько параллельных потоков внутри БД при работе внутри одного логического кода (программы).
...
> * отсев не изменившихся данных (снижает нагрузку на ЦП процентов на 80);
...
К этому проекту я шёл почти 9 лет (с конца 2014 года). Считаю, что данный проект на Си полностью реализовал возложенные на него надежды. Более того, он в разы превзошёл все самые смелые ожидания.
<p> &nbsp; </p> 
<p> &nbsp; </p> 
...
> *   С 2019 стал работать с Python. С 2019 немного Python на стороне БД PostgreSQL. С 2020 много Python на стороне сайта [сайт на голом Питоне (не фреймворки)]. 
...
> *   Поэтому с января 2023 пошёл на курсы языка Си в образовательную платформу otus.ru. Изучил всё что хотел и даже в несколько раз больше. Узнал очень много технической документации. 
